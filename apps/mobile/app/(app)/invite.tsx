...
export default function InviteScreen() {
  ...
  useEffect(() => {
    ...
    createInvite.mutate(undefined, {
      ...
      onError: (err: any) => {
        Alert.alert(
          'Erro',
          err?.response?.data?.message ?? 'Não foi possível gerar o convite.',
  ...
  const handleRegenerate = async () => {
    if (!email.trim()) {
      Alert.alert('Email vazio', 'Digite o email do cônjuge.');
      return;
    }
    try {
      ...
      Alert.alert('Erro', err?.response?.data?.message ?? 'Falhou.');
    }
  };
